	...
	for (i = 0; i < len; i++) {
		...
		switch (ch){	// 아래의 case문대로 조건실행, 혹은 default에 따라 처리함.
			...
			// 스택에 있는 연산자의 우선순위가 더 크거나 같으면 출력
				while (!is_empty(&s) && (prec(ch) <= prec(peek(&s)))) { //비교대상 연산자랑 스택에 있는 연산자랑 비교해서 비교대상이 우선순위 낮거나 같으면 계속 pop함
					str[k++] = pop(&s); 
					//printf("%c", pop(&s));
				}
				push(&s, ch);   //그다음에 무조건 비교대상 ch는 push함
				break;
			case '(':
				push(&s, ch);   //괄호는 무조건 push함
				break;
			case ')':
...
int eval(char exp[]){
	...
	init(&s);	// 스택 초기화
	for (i = 0; exp[i] != '\0'; i++){	//deque와 동일. exp에서 deque해 stack에 push.
		ch = exp[i];
		if (ch != '+' && ch != '-' && ch != '*' && ch != '/'){
